For people who read all tweets on Twitter time-line, show all unread tweets in reverse chronological order.
You can also apply this add-on only to a specified list.

This add-on works on Twitter home, so you can use with other useful add-ons and greasemonkey scripts such as @troynt's Twitter Script( http://userscripts.org/scripts/show/40617 ),Å@whose features are url expansion and media embed and so on.

Other functions
-Show replies from users you don't follow, too(you can turn off this setting)
-Notify new DMs, too
